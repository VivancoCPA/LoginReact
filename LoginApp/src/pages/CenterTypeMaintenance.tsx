...
import { CenterTypeDrawer } from '../components/CenterTypeDrawer';
import { ConfirmDialog } from '../components/ConfirmDialog';
import type { CenterTypeItem } from '../types/centerType';
...
export const CenterTypeMaintenance: React.FC = () => {
  // Persistent layout mode (table vs cards)
  const [viewMode, setViewMode] = useState<LayoutMode>(() => {
    const saved = sessionStorage.getItem('centerTypesLayoutSelection');
    return saved === 'cards' ? 'cards' : 'table';
  });

  const [centerTypes, setCenterTypes] = useState<CenterTypeItem[]>([]);
  ...
  const [selectedCenterType, setSelectedCenterType] = useState<CenterTypeItem | null>(null);
  ...
  const [centerTypeToToggle, setCenterTypeToToggle] = useState<CenterTypeItem | null>(null);
  const [isTogglingStatus, setIsTogglingStatus] = useState(false);

  // Persist layout selection
  useEffect(() => {
    sessionStorage.setItem('centerTypesLayoutSelection', viewMode);
  }, [viewMode]);
  ...
  // Fetch center types data from API
  const fetchCenterTypes = useCallback(async () => {
    setIsLoading(true);
    try {
      // Map sort fields to backend supported sorting keys ('name', 'isActive', 'created_at', 'updated_at')
      ...
      setCenterTypes(data.items || []);
      setTotalCount(data.totalCount || 0);
      setTotalPages(data.totalPages || 1);
    } catch (error) {
      console.error('Error loading center types:', error);
      toast.error('No se pudo cargar el listado de tipos de centros médicos.');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleOpenDrawer = (mode: 'view' | 'create' | 'edit', centerType: CenterTypeItem | null) => {
  ...
  const handleToggleStatusClick = (centerType: CenterTypeItem, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();

    if (centerType.isActive) {
      // Prompt confirm dialog on logical deactivation
      setCenterTypeToToggle(centerType);
      setIsConfirmOpen(true);
    } else {
      // Activate immediately
      toggleStatus(centerType);
    }
  };

  const toggleStatus = async (centerType: CenterTypeItem) => {
    setIsTogglingStatus(true);
    try {
      const result = await centerTypeService.toggleCenterTypeStatus(centerType.id);
      toast.success(
        `Tipo de centro "${centerType.name}" ${result.isActive ? 'activado' : 'inactivado'} correctamente.`
      );
      setIsConfirmOpen(false);
      setCenterTypeToToggle(null);
      fetchCenterTypes();
    } catch (error) {
      console.error('Error toggling center type status:', error);
      toast.error('No se pudo modificar el estado del tipo de centro.');
    } finally {
  ...
  return (
    <div className="flex flex-col h-full w-full text-left overflow-hidden gap-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 uppercase tracking-wide">
            Mantenimiento de Tipos de Centro
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 select-none">
            Gestione los tipos de centros médicos que estarán disponibles para su asignación en la plataforma.
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 pr-1 -mr-1">
        ...
        <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-16 shadow-sm text-center select-none">
          ...
          <h3 className="mt-4 text-sm font-semibold text-slate-800 dark:text-slate-200">
            No se encontraron tipos de centros
          </h3>
        ...
        // VIEW MODE: HIGH-DENSITY COMPACT TABLE (NO AVATAR COLUMN)
              ...
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950/20 border-b border-slate-200 dark:border-slate-800 text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-bold select-none">
                  <th
                    ...
                    className="px-6 py-3.5 text-left cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors"
                  >
                  ...
                  <th className="px-6 py-3.5 text-left">Registrado</th>
                  <th className="px-6 py-3.5 text-right w-[150px]">Acciones</th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-slate-150 dark:divide-slate-800/60">
                {filteredCenterTypes.map((type) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-2.5 text-left text-xs font-mono text-slate-650 dark:text-slate-350 select-all align-middle">
                      {formatDateToYYYYMMDD(type.createdAt)}
                    </td>

                    <td className="px-6 py-2.5 text-right align-middle select-none">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          type="button"
                          onClick={() => handleOpenDrawer('view', type)}
                        ...
                        <button
                          type="button"
                          onClick={() => handleOpenDrawer('edit', type)}
                        ...
                        <button
                          type="button"
                          onClick={(e) => handleToggleStatusClick(type, e)}
        ...
        // VIEW MODE: HIGH-FIDELITY RESPONSIVE CARDS GRID VIEW (NO AVATARS)
          ...
          {filteredCenterTypes.map((type) => (
            ...
            >
              <div>
                ...
                <div className="flex items-start justify-between gap-3 select-none">
                  ...
                  <div className="relative shrink-0 flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveMenuId(activeMenuId === type.id ? null : type.id);
                      }}
                      className="p-1 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-350 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors focus:outline-none"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4">
                      ...
                      <>
                        ...
                        <div className="absolute right-0 mt-30 w-40 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-1.5 shadow-xl z-20 animate-fadeIn select-none">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              setActiveMenuId(null);
                              handleToggleStatusClick(type, e);
                            }}
                ...
                <div className="mt-4 space-y-2 text-xs text-left">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-705 dark:text-slate-350 font-mono font-medium">{type.id}</span>
                  </div>
                </div>
              ...
              <div className="mt-6 border-t border-slate-100 dark:border-slate-800/40 pt-3 text-[10px] text-slate-400 dark:text-slate-550 select-none flex justify-between">
                <span>Registrado: {formatDateToYYYYMMDD(type.createdAt)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4 select-none shrink-0 border-t border-slate-100 dark:border-slate-800/60 pt-4">
        <span className="text-xs text-slate-500 dark:text-slate-400">
          ...
          <span className="font-semibold text-slate-700 dark:text-slate-300">
            {filteredCenterTypes.length}
          </span>{' '}
          de{' '}
          <span className="font-semibold text-slate-700 dark:text-slate-300">
            {totalCount}
          </span>{' '}
          tipos de centros en total
        </span>
      ...
      {/* Slide-over CenterType details / CRUD form Drawer Panel */}
      <CenterTypeDrawer
        isOpen={isDrawerOpen}
        onClose={() => setIsDrawerOpen(false)}
        mode={drawerMode}
        centerType={selectedCenterType}
        onSaveSuccess={fetchCenterTypes}
      />
      ...
      <ConfirmDialog
        ...
        title="Desactivar Tipo de Centro"
        message="Este tipo de centro ya no estará disponible para nuevos centros médicos. ¿Desea continuar?"
...
export default CenterTypeMaintenance;
