...
export const RoleMaintenance: React.FC = () => {
  ...
  // Sorting State
  const [sortField, setSortField] = useState<'name' | 'assignedUsersCount'>('name');
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      const cleanedData = data.map((r) => ({
        ...
        isSystemRole: r.name === 'Admin' || r.name === 'User' || r.name === 'Auditor' || r.name === 'Asegurador',
      }));
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-16 text-center shadow-xs select-none">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 max-w-sm mx-auto">
            No encontramos ningún rol del sistema registrado que coincida con el criterio de búsqueda "{searchQuery}".
          </p>
        </div>
      ) : layoutMode === 'table' ? (
        // TABLE MODE VIEW
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      {sortField === 'name' ? (
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 opacity-20">
                  ...
                  <tr key={role.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                    ...
                    <td className="px-6 py-3.5">
                      <div className="flex items-center gap-2 max-w-sm">
                        <span className="font-bold text-slate-800 dark:text-slate-150 truncate">{role.name}</span>
                        {role.isSystemRole && (
                    ...
                    <td className="px-6 py-3.5">
                      <span className="inline-flex items-center justify-center px-2.5 py-1 text-xs font-semibold rounded-lg bg-emerald-50 dark:bg-emerald-600/10 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-600/20">
                        {role.assignedUsersCount} {role.assignedUsersCount === 1 ? 'Usuario' : 'Usuarios'}
                      </span>
                    </td>
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4.5 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/30 dark:bg-slate-950/5 flex items-center justify-between select-none">
              <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">
                Mostrando del {startIndex + 1} al {Math.min(startIndex + pageSize, totalItems)} de {totalItems} roles
              </span>
            ...
            {paginatedRoles.map((role) => (
              ...
              >
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-0.5 text-left max-w-[80%]">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <h4 className="font-extrabold text-base text-slate-850 dark:text-slate-100 truncate">{role.name}</h4>
                      {role.isSystemRole && (
                  ...
                  <div className="relative select-none">
                    ...
                    {activeMenuId === role.id && (
                      <div className="absolute right-0 mt-1 w-32 bg-white dark:bg-slate-850 border border-slate-100 dark:border-slate-800/80 rounded-xl shadow-xl z-20 py-1.5 text-left transition-all duration-200">
                        <button
                          type="button"
                          onClick={() => openEditDrawer(role)}
                ...
                <div className="mt-5 border-t border-slate-100 dark:border-slate-800/60 pt-4 flex items-center justify-between text-xs select-none">
                  <span className="text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider text-[10px]">Asignación</span>
