...
interface CenterTypeDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  mode: 'view' | 'create' | 'edit';
  centerType: CenterTypeItem | null;
  onSaveSuccess: () => void;
}
...
export const CenterTypeDrawer: React.FC<CenterTypeDrawerProps> = ({
  isOpen,
  onClose,
  mode: initialMode,
  centerType,
  ...
  // Sync state when drawer opens, centerType changes, or mode changes
  useEffect(() => {
    if (isOpen) {
      setDrawerMode(initialMode);
      if ((initialMode === 'edit' || initialMode === 'view') && centerType) {
        setName(centerType.name || '');
        setIsActive(centerType.isActive !== false);
      } else {
        setName('');
        setIsActive(true);
      }
  ...
  }, [isOpen, initialMode, centerType]);

  // Sync drawer mode with itself if initial mode changes internally
  useEffect(() => {
    setDrawerMode(initialMode);
  }, [initialMode]);
  ...
  const validate = () => {
    ...
    if (!name.trim()) {
      newErrors.name = 'El nombre del tipo de centro es obligatorio.';
    } else if (name.trim().length < 3) {
      newErrors.name = 'El nombre del tipo de centro debe tener al menos 3 caracteres.';
    } else if (name.trim().length > 100) {
      newErrors.name = 'El nombre del tipo de centro no puede exceder los 100 caracteres.';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (drawerMode === 'create') {
        await centerTypeService.createCenterType({
          name: name.trim(),
        });
        toast.success('Tipo de centro médico creado exitosamente.');
      } else {
        if (!centerType) return;
        await centerTypeService.updateCenterType(centerType.id, {
          name: name.trim(),
          isActive: isActive,
        });
        toast.success('Tipo de centro médico actualizado exitosamente.');
      }
      onSaveSuccess();
      onClose();
    } catch (err: any) {
      ...
      if (err.response && err.response.status === 409) {
        msg = 'Ya existe un tipo de centro registrado con este nombre.';
    ...
    <div className="fixed inset-0 z-50 flex justify-end overflow-hidden animate-fadeIn">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col h-full z-10 transition-transform duration-300 ease-out transform translate-x-0">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wide">
              {drawerMode === 'create' 
                ...
                : drawerMode === 'edit' 
                ? 'Editar Tipo de Centro' 
                : 'Detalle de Tipo de Centro'}
            </h3>
            {centerType && (
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar text-left">
          ...
          {drawerMode === 'view' && centerType ? (
            /* VIEW MODE DETAILS - PURE TEXT, NO AVATAR */
            <div className="space-y-6 select-none">
              <div className="space-y-4">
                <div className="border-b border-slate-100 dark:border-slate-800/60 pb-3">
                  ...
                  <span className="block mt-1 text-sm font-semibold text-slate-800 dark:text-slate-150 font-medium">
                    {centerType.name}
                  </span>
                </div>
                ...
                <div className="pb-3">
                  ...
                  <span className="block mt-1 text-sm font-mono font-medium text-slate-700 dark:text-slate-300">
                    {formatDateToYYYYMMDD(centerType.createdAt)}
                  </span>
                </div>
              </div>
            </div>
                ...
                <div className="flex flex-col gap-1.5 text-left">
                  ...
                  >
                    ...
                    <div className="space-y-0.5">
                      ...
                      >
                        Tipo de Centro Activo
                      </span>
                      <span className="block text-[10px] text-slate-400 dark:text-slate-500 leading-relaxed font-medium">
                        Active esta opción para habilitar el tipo de centro médico en el catálogo del sistema.
                      </span>
                    </div>
                  </div>
                </div>
...
export default CenterTypeDrawer;
